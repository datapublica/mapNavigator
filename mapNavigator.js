...
MapNavigator.prototype.computeSeries = function (data) {
    // Init vars
    var values = [], min = null, max = null, config = this.config;
...
MapNavigator.prototype.draw = function (zones, data) {
    ...
    function drawPies(x, y, areas, seriesValues, radiusRatio) {
        ...
        var pieRadius = MapNavigator.closestPair(points).distance * radiusRatio / 2.1;
    ...
    if (config.exporting.enabled) {
        ...
        sensibleZone.click(function () {
            ...
            svg = svg.replace(/stroke-width="1"/g, 'stroke-width="' + (1 / config.exporting.scale) + '"');
